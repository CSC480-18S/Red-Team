...
public class PlayerTests {
  ...
  */
  ...
  /**
  * performs a GET request on a player
  * given a username that is empty
  */
  @Test
  public void playerWithEmptyName() throws Exception {
    MockHttpServletRequestBuilder builder = get("/players/search/findByUsername?username=");
    builder.contentType(MediaType.APPLICATION_JSON_UTF8);
    MvcResult res = mvc.perform(builder).andReturn();
    ObjectMapper mapper = new ObjectMapper();
    JsonNode root = mapper.readTree(res.getResponse().getContentAsByteArray());
  ...
  /**
  * performs a GET request on a player
  * given a username that is null
  */
  @Test
  public void playerWithNullName() throws Exception {
    MockHttpServletRequestBuilder builder = get("/players/search/findByUsername?username=" + null);
    builder.contentType(MediaType.APPLICATION_JSON_UTF8);
    MvcResult res = mvc.perform(builder).andReturn();
    ObjectMapper mapper = new ObjectMapper();
    JsonNode root = mapper.readTree(res.getResponse().getContentAsByteArray());
